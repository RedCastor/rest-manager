...
class Rest_Manager_Settings {
   ...
   * @return array
   */
  private function get_default_field_descriptor( $field ) {

    $default_field = array(
      'global'            => isset($field['global']) ? $field['global'] : false,
      'name'              => $field['name'],
      'encode'            => isset($field['encode']) ? $field['encode'] : false,
   ...
   * @return array
   */
  private function get_default_section_descriptor( $section ) {
  ...
  public static function createInstance( $name, $version ) {
    ...
    if(!isset(self::$_instance[$name])) {
      self::$_instance[$name]['instance'] = new Rest_Manager_Settings( $name, $version );
    }
  ...
  public function register_fields( $fields ) {
    ...
    foreach ($fields as $tab_key => $tabs) {
      ...
      foreach ($tabs['sections'] as $section_key => $section) {
        ...
        $default_section = $this->get_default_section_descriptor( $section );
        ...
        foreach ($section['fields'] as $field ) {
          ...
          $default_field = $this->get_default_field_descriptor( $field );
  ...
  public function get_option_plugin_supports ( $option ) {
    ...
    if (isset($_rest_manager_plugin_features[ $option_name ][0])) {
      return $_rest_manager_plugin_features[ $option_name ][0];
    }
  ...
  public function is_disabled ( $option, $global = true, $section_key = '' ) {
    ...
    if ($global === true) {
      ...
      else if ( current_theme_supports( $option_name ) || rest_manager_plugin_supports( $option_name ) ) {
      ...
      if ( current_theme_supports( $option_name ) || rest_manager_plugin_supports( $option_name ) ) {
  ...
  public function get_conditions () {
    ...
    $conditions = array(
      'is_home'               => __('Is Home',                'rest-manager'),
      'is_front_page'         => __('Is Front page',          'rest-manager'),
      'is_home&is_front_page' => __('Is Home and Front page', 'rest-manager'),
      'is_single'             => __('Is Single',              'rest-manager'),
      'is_sticky'             => __('Is Sticky',              'rest-manager'),
      'is_page'               => __('Is Page',                'rest-manager'),
      'is_page_template'      => __('Is Page Template',       'rest-manager'),
      'is_category'           => __('Is Category',            'rest-manager'),
      'is_tag'                => __('Is Tag',                 'rest-manager'),
      'is_tax'                => __('Is Taxonomy',            'rest-manager'),
      'is_author'             => __('Is Author ',             'rest-manager'),
      'is_archive'            => __('Is Archive',             'rest-manager'),
      'is_search'             => __('Is Search',              'rest-manager'),
      'is_404'                => __('Is 404',                 'rest-manager'),
      'is_singular'           => __('Is Singular',            'rest-manager'),
      'is_user_logged_in'     => __('Is User Logged In',      'rest-manager'),
    );

    //Get Page list with page id in string value
    foreach ( wp_list_pluck(get_pages(), 'post_title', 'ID') as $page_id => $page_title) {
      $conditions['is_page$' . strval($page_id)] = sprintf( '%s %s', __('Page', 'rest-manager'), $page_title);
    }

    //Get all page templates list
    $templates = get_page_templates();
    foreach ( $templates as $template_name => $template_filename ) {
      $conditions[ 'is_page_template$' . $template_filename] = sprintf( '%s %s', __('Page Template ', 'rest-manager'), $template_name);
    }
    ...
    foreach ( $post_types as $post_type ) {
      $conditions[$post_type->name] = sprintf( '%s %s', __('Post Type ', 'rest-manager'), $post_type->name);
    }

    //Woocommerce conditions
    if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {

      $conditions['is_woocommerce']       = __('Is Woocommerce',  'rest-manager');
      $conditions['is_shop']              = __('Is Shop',         'rest-manager');
      $conditions['is_product_category']  = __('Is Product Category', 'rest-manager');
      $conditions['is_product_tag']       = __('Is Product Tag',  'rest-manager');
      $conditions['is_product']           = __('Is Product',      'rest-manager');
      $conditions['is_cart']              = __('Is Cart',         'rest-manager');
      $conditions['is_checkout']          = __('Is Checkout',     'rest-manager');
      $conditions['is_account_page']      = __('Is Account Page', 'rest-manager');
      $conditions['is_wc_endpoint_url']   = __('Is Endpoint Url', 'rest-manager');
      $conditions['is_ajax']              = __('Is Ajax',         'rest-manager');
    }
  ...
  public function set_default_field_args( $field_args, $tab_key, $section_key ) {
    ...
    if ( isset($field_args['encode']) && is_string($field_args['encode']) ) {
      $name = call_user_func($field_args['encode'], $name);
    }

    if ( isset($field_args['global']) && $field_args['global'] === true) {
      $disable = $this->is_disabled( $field_args['name'] ) ? true : $disable;
      $name_id = $this->get_option_prefix( $name );
    }
    else if ( isset($field_args['parent']) ) {
      $disable = $this->is_disabled( $field_args['name'], false, $section_key ) ? true : $disable;
      $name_id = $field_args['parent']['name_id'] . '[' . $name . ']';
    }
    else {
      $disable = $this->is_disabled( $field_args['name'], false, $section_key ) ? true : $disable;
      $name_id = $this->get_option_prefix($section_key . '[' . $name . ']');
    }

    $args = array(
      'id'          => $field_args['name'],
      'name_id'     => $name_id,
      'label_for'   => "{$section_key}[{$field_args['name']}]",
  ...
  /**
   * Get field modal for display
   *
   * @param $args
   *
   * @return string
   */
  public function get_field_modal_multiselect( $type, $args, $title, $button ) {
    ...
    $options = call_user_func(array($this, 'get_' . $type));

    $multiselect_html = sprintf( '<select multiple="multiple" class="%1$s-multi-select-field" name="%2$s[]" id="%3$s_%4$s" %5$s>', $this->settings_prefix, $name_id, $elment_id, $type, $disable );
    ...
    $html = sprintf( '<a href="#TB_inline?&height=370&width=470&inlineId=%1$s_modal_%2$s" title="%3$s" class="button button thickbox">%3$s</a>', $this->settings_prefix, hash('md5', $elment_id), $button );
    $html .= sprintf( '<div id="%1$s_modal_%2$s" style="display:none;"><p>%3$s</p></div>', $this->settings_prefix, hash('md5', $elment_id), $multiselect_html );
  ...
  public function callback_checkbox( $args ) {
    ...
    switch ( $args['id'] ) {
      case 'plugins':
        $modal_title = __('Select plugins', 'rest-manager');
        $button_name = $modal_title;
        break;
      case 'conditions':
        $modal_title = __('Select conditions', 'rest-manager');
        $button_name = $modal_title;
        break;
    }

    if ( $args['id'] === 'plugins' || $args['id'] === 'conditions' ) {
      $name_id .= '[value]';
      $value = isset($value['value']) ? $value['value'] : 'off';
      $args['name_id'] .= '[options]';
      $html_content = $this->get_field_modal_multiselect( $args['id'], $args, $modal_title, $button_name );
    }
    ...
    foreach ( (array) $wp_settings_sections[$page] as $section ) {
      ...
      switch ( $display ) {
        case 'table':
          ...
          echo '<th scope="col" id="options" class="manage-column column-options">' . __('Options', 'rest-manager') . '</th>';
          echo '</tr>';
          echo '</thead>';
